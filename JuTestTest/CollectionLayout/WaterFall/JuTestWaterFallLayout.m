...
CGFloat const colCounts = 2;
@implementation JuTestWaterFallLayout
...
- (void)prepareLayout{
    ...
    float top = 0.0;
//    = [self inset:[NSIndexPath indexPathForRow:0 inSection:0]].top;

    NSInteger column=MAX(colCounts, _ju_columnCount);

     for (int i = 0; i < column; i++) {
         NSIndexPath *indexPath=[NSIndexPath indexPathForRow:i inSection:0];
         UICollectionViewLayoutAttributes *attribute = [super layoutAttributesForItemAtIndexPath:indexPath];
         if (top==0) {
             top=attribute.frame.origin.y;
         }
         else{
             top=MIN(attribute.frame.origin.y, top);
         }
    }
...
-(UICollectionViewLayoutAttributes*)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *rectStr=_ju_dicAttribute[[self stringWithIndex:indexPath]];
...
- (void)layoutItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [_ju_dicAttribute setValue:NSStringFromCGRect(frame) forKey:[self stringWithIndex:indexPath]];
}
-(NSString *)stringWithIndex:(NSIndexPath *)indexPath{
...
/*- (NSArray *)indexPathsOfItem:(CGRect)rect{
    ...
    for (NSString *rectStr in _ju_dicAttribute) {
        NSArray *arrIndex=[rectStr componentsSeparatedByString:@","];
        NSIndexPath *indexPath=[NSIndexPath indexPathForRow:[arrIndex.firstObject integerValue] inSection:[arrIndex.lastObject integerValue]];
        CGRect cellRect = CGRectFromString(_ju_dicAttribute[rectStr]);
